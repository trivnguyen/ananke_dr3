...
def parse_cmd():
    ...
    parser.add_argument('-b', '--num-batch', type=int, help='number of batch', default=1)
...
submit_dir = f"slurm_submit/fix_astrometric/{gal}-lsr-{lsr}-rslice-{rslice}"
os.makedirs(submit_dir, exist_ok=True)

print(f'Galaxy, LSR, rslice: {gal}, {lsr}, {rslice}')

# Define input and output paths
run_cmd = "srun -n1 -N1 python fix_astrometric.py "\
    f"--gal {gal} --lsr lsr-{lsr} --rslice {rslice} --num-batch {num_batch}"
